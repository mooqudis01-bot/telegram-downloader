...
async def api_send_otp(req: SendOtpRequest):
    ...
    if success:
        ...
        raise HTTPException(status_code=400, detail=error_msg or "Failed to send OTP")
...
async def api_verify_otp(req: VerifyOtpRequest):
    ...
    if status == "SUCCESS":
        return {"success": True, "need_2fa": False, "user": user_info}
    elif status == "NEED_2FA":
        return {"success": True, "need_2fa": True}
    else:
        raise HTTPException(status_code=400, detail=error_msg or "Verify OTP failed")
...
async def api_verify_2fa(req: Verify2faRequest):
    ...
    if success:
        ...
        raise HTTPException(status_code=400, detail=error_msg or "Invalid 2FA password")
